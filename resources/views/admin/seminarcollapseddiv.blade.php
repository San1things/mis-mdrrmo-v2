        ...
        <form method="get">
            <div class="input-group m-3 px-5 pb-3">
                ...
                <a class="btn btn-outline-secondary fs-3 px-4 py-2" id="clear-btn"
                    href="/seminarcollapseddiv?id={{ $seminar->id }}">Clear</a>
            </div>
        </form>
                ...
                @forelse ($attendees as $attendee)
                    <div class="col-5 position-relative pt-5 pe-5 ps-3 border m-3">
                        ...
                        @if ($seminar->status == 'upcoming')
                            <button class="btn btn-danger fs-2 position-absolute top-0 end-0 me-3 mt-3 remove-attendee-btn "
                                data-uid="{{ $attendee->user_id }}" data-sid="{{ $seminarid }}" data-bs-toggle="modal"
